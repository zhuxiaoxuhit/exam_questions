...
11. ⚠️ 【禁止】题干和选项中严禁出现章节编号或书籍目录信息
   - ❌ 不能出现"5.2.4"、"第3章"、"3.1.2"等章节编号
   - ❌ 不能出现"根据5.2.4规定"、"按照第3章要求"等引用章节的表述
   - 原因：这些章节信息来自参考书目录，会引导学生查找书籍而非考查知识点本身
   - ✅ 正确做法：直接陈述知识点内容，不引用章节号
   - 📌 示例：
     * ❌ 错误："根据5.2.4规定，上柱插筋应包括在（   ）中"
     * ✅ 正确："因柱变截面处上柱插筋与下柱钢筋一同安装，故上柱插筋应包括在（   ）中"
...
1. **有效性检查和重复性检查**（⚠️ 最重要）
   ...
   - ⚠️ 【禁止】是否出现了"例如"题或"举例"题？
     ...
     * 违反此项必须标记为严重问题
   - ⚠️ 【禁止】题干和选项中是否出现了章节编号？
     * ❌ 检查是否包含"5.2.4"、"第3章"、"3.1.2"等章节编号
     * ❌ 检查是否有"根据5.2.4规定"、"按照第3章要求"等表述
     * 原因：章节信息会引导学生查找书籍而非考查知识点本身
     * 违反此项必须标记为严重问题
